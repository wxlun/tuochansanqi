...
class ClientHandle(object):
    ...
    def load_latest_config(self):
        '''
        load the lastest monitor configs from monitor server
        :return:
        '''
    ...
    def forever_run(self):
        ...
        while not exit_flag:
            if time.time() - config_last_update_time > settings.configs['ConfigUpdateInterval']:
                self.load_latest_config()  #获取最新的监控配置信息
                print("Loaded lastest config:",self.monitored_services)
            ...
            for service_name,val in self.monitored_services['services'].items():
                if len(val) == 2: #means it's the first time to monitor
                    self.monitored_services['services'][service_name].append(0) #为什么是0，因为为了保证第一次肯定触发监控这个服务
